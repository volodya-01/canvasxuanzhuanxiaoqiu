		...
		while (p != null) {
			...
			*/
			...
			if (outsideTest||p.dead) {
				...
				if (p.flash == 0) {
				    context.strokeStyle = "rgba(0,147,255," + depthAlphaFactor * p.alpha + ")";
				    context.fillStyle = "rgba(0,147,255," + depthAlphaFactor * p.alpha + ")";
				}
				else {
				    context.strokeStyle = "rgba(255,0,0," + depthAlphaFactor * p.alpha + ")";
				    context.fillStyle = "rgba(255,0,0," + depthAlphaFactor * p.alpha + ")";
                    p.flash--;
				}
				context.lineWidth = 1;
				//draw
				...
				r = p.flash / 10;
				if (r < 1) r = 1;
				context.arc(p.projX, p.projY, m * particleRad * r, 0, 2 * Math.PI, false);
